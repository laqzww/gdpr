...
async function handleSaveAttachment(preparedId, attachmentId) {
    ...
    const card = detailEl.querySelector(`.prepared-response[data-prepared-id="${preparedId}"]`);
    if (!card) return;
    const container = card.querySelector(`.attachment-block[data-attachment-id="${attachmentId}"]`);
    if (!container) return;
    const textArea = container.querySelector('textarea[data-role="attachment-text"]');
...
async function handleSaveMaterial(materialId) {
    ...
    if (!card) return;
    const textArea = card.querySelector('textarea[data-role="material-text"]');
    const approvedCheckbox = card.querySelector('[data-role="material-approved"]');
